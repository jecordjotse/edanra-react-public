...
const DashboardAdCard = ({ property, removePropertyStart, allProperties }) => {
  ...
  const subscription_type = property.subscription_type
    ? property.subscription_type
    : "free_three";
  const renew_date = last_date_paid
    ? new Date(last_date_paid)
    : new Date("2020-10-31");
  ...
  const { status, daysLeft } = dateChecker(
          ...
          {deleteToggler ? (
            <>
              ...
              <div className="options">
                ...
                <p>
                  <a onClick={handleDelete} className="options-link" href="#">
                    Yes
                  </a>
                </p>
                {/* eslint-disable-next-line jsx-a11y/anchor-is-valid */}
            ...
            <div className="options">
              <p>
                <Link className="options-link" to={`/edit-space/${routeName}`}>
                  Edit
                </Link>
              </p>
              {/* eslint-disable-next-line jsx-a11y/anchor-is-valid */}
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            {status === "Hosted" ? (
              <span style={{ paddingRight: 16 }}>
                Days remaining:{" "}
                <span style={{ color: "#004D44" }}>{daysLeft}</span>
              </span>
            ) : (
              <span style={{ paddingRight: 16 }}>
                <NavLink
                  style={{ color: "#fb9800", cursor: "pointer" }}
                  to={`/payment/${property.uid}`}
                >
                  Pay to Host
                </NavLink>
              </span>
