export const getSearchResults = ({ filters, allProperties }) => {
  if (
    filters.town === filters.region &&
    filters.town !== "" &&
    filters.region === filters.district &&
    filters.region !== "" &&
    filters.town === filters.district &&
    filters.town !== ""
  ) {
    console.log("here");
    return allProperties.filter((property) => {
      return (
        property.town.toLowerCase().includes(filters.town.toLowerCase()) ||
    ...
    const transformedFilters = {
      property_type: (property_type) =>
        property_type.includes(filters.property_type),
